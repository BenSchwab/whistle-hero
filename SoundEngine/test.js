...
//sLoader.loadSound("SoundEngine/Whistles/3.wav",context).then(function(buffer){playWhistle(buffer);});
getLiveInput();
onError =function(e){
   console.log("Error: ", e);
};
...
window.addEventListener("whistle.detect", onWhistle);
function onWhistle(e){
  console.log("got it!");
  onWhistle.h2 = onWhistle.h2 || document.querySelector("h2");
  onWhistle.h2.innerHTML = "whistle "+ e.detail.intensity +" freq: " + e.detail.frequency;
}
...
function update2(data) {
    //webkitRequestAnimationFrame(update2);
    if(!setup) return;
    gfx2.clearRect(0,0,6000,600);
    gfx2.fillStyle = 'gray';
    gfx2.fillRect(0,0,6000,600);
    ...
    for(var i=0; i<data.length; i++) {
        gfx2.fillRect(100+i*4,100+256-data[i]*8000,3,100);
    }
